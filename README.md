...
| `timing` | A ball sweeps back and forth along a line that has a ring on it. | Press while the ball is inside the ring; the ring then jumps somewhere else. Land 10 hits. There's one chance per ring: pressing while the ball is outside it is a miss, and so is letting the ball sweep through it without a press. |
...
| `django` (aliases `western`, `quickdraw`, `shooting`) | A quick-draw shooting gallery in a western town. Each wave, 3–5 targets pop up at random spots, with a fuse burning along the bottom. | Click every target before the fuse runs out. You get exactly one round per target, so a single miss ends the run. Clear 10 waves. |
